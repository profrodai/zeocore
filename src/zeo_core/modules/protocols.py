...
This module defines the Protocol interfaces for modules in the ZeoCore system,
...
class ZeoPluginMetadata(BaseModel):
    """
    Metadata for ZeoCore modules.
...
@runtime_checkable
class ZeoPluginProtocol(Protocol):
    """
    Base protocol for all ZeoCore modules.
    ...
    def get_metadata(self) -> ZeoPluginMetadata:
        ...
        Returns:
            ZeoPluginMetadata: Plugin metadata including id, version, capabilities
        """
        ...
...
class PluginRegistryProtocol(Protocol):
    ...
    def register(self, plugin: ZeoPluginProtocol) -> None:
        ...
        Raises:
            ZeoPluginError: If plugin is already registered
        """
        ...

    def get_plugin(self, plugin_id: str) -> ZeoPluginProtocol | None:
        """
...
class PluginLoaderProtocol(Protocol):
    ...
    def load_plugin(self, module_path: str) -> ZeoPluginProtocol:
        ...
        Raises:
            ZeoPluginError: If plugin cannot be loaded
        """
        ...
...
@runtime_checkable
class CommandPluginProtocol(ZeoPluginProtocol, Protocol):
    """
...
@runtime_checkable
class WorkflowPluginProtocol(ZeoPluginProtocol, Protocol):
    """
...
@runtime_checkable
class ExtensionPluginProtocol(ZeoPluginProtocol, Protocol):
    """
...
@runtime_checkable
class ProviderPluginProtocol(ZeoPluginProtocol, Protocol):
    """
...
@runtime_checkable
class ConfigurablePluginProtocol(ZeoPluginProtocol, Protocol):
    """
