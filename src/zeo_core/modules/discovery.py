...
class PluginInfo(ZeoPluginMetadata):
    """
    Plugin information used for validation.

    Extends ZeoPluginMetadata with stricter requirements:
...
class PluginLoader:
    ...
    def _validate_plugin(
        self, plugin: ZeoPluginProtocol, module_path: str
    ) -> ZeoPluginProtocol:
        """
        ...
        Raises:
            ZeoPluginError: If validation fails
        """
        try:
            ...
            if not hasattr(plugin, "get_metadata") or not callable(plugin.get_metadata):
                ...
                if not isinstance(metadata, ZeoPluginMetadata):
                    if isinstance(metadata, dict):
                        metadata = ZeoPluginMetadata(**metadata)
                    else:
                        raise TypeError(
                            f"get_metadata() must return a ZeoPluginMetadata object, "
                            f"got {type(metadata)}"
                        )
        ...
        for name, obj in inspect.getmembers(module):
            if inspect.isclass(obj) and name == class_name:
                try:
                    ...
                    # `object`, not the ZeoPluginProtocol this class is
                    ...
                    candidate = cast(ZeoPluginProtocol, obj())
    ...
    def load_plugin(self, module_path: str) -> ZeoPluginProtocol:
        """
        ...
        Raises:
            ZeoPluginError: If the plugin cannot be loaded
        """
    ...
    def load_plugins(self, modules: list[str]) -> list[ZeoPluginProtocol]:
        """
        ...
        """
        plugins: list[ZeoPluginProtocol] = []

        for module_path in modules:
            try:
                plugin = self.load_plugin(module_path)
                plugins.append(plugin)
            except ZeoPluginError as e:
                self.logger.error(
    ...
    def load_entry_points(
        self, group: str = "zeo_core.modules"
    ) -> list[ZeoPluginProtocol]:
        """
    ...
    def load_enabled_entry_points(
        ...
        """
        from zeo_core.modules.registry import registry
    ...
    def load_enabled_modules(
        ...
        Args:
            modules: List of module paths (e.g., ["zeo_core.core.fs.plugin"])
            strict: If True, fail fast on first error with rollback
            auto_register: If True, automatically register loaded modules
    ...
    def discover_plugins(
        self,
        entry_point_group: str = "zeo_core.modules",
        ...
        plugins: list[ZeoPluginProtocol] = self.load_entry_points(entry_point_group)
