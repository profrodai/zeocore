...
class LoggingConfig(BaseModel):
    ...
    def setup_logging(self) -> None:
        ...
        from zeo_core.core.logging import LOG_LEVELS, LogLevel, configure_logger

        level_name = self.level.upper()
        level = LOG_LEVELS.get(LogLevel(level_name), LOG_LEVELS[LogLevel.INFO])

        logger = configure_logger("zeocore", level=level, log_file=self.file)
...
class GeneralConfig(BaseModel):
    ...
    project_name: str = Field(default="ZeoCore", description="Name of the project")
...
class ZeoConfig(BaseModel):
    """Main configuration for zeo_core."""
