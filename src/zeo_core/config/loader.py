...
def load_yaml_config(path: str) -> dict[str, Any]:
    ...
    Raises:
        ZeoConfigurationError: If the file cannot be loaded/parsed.
    """
    try:
        ...
        raise ZeoConfigurationError(f"Failed to load YAML config: {e}", path) from e
...
def find_config_file() -> str | None:
    ...
    if config_path := os.environ.get("ZEO_CONFIG"):
    ...
    for name in ["zeo_config.yaml", "config/zeo_config.yaml"]:
...
def load_config(
    ...
    Returns:
        A ZeoConfig instance.

    Raises:
        ZeoConfigurationError: If explicit config path is invalid.
    """
    ...
    return ZeoConfig.model_validate(config_dict)


def merge_configs(base: ZeoConfig, override: dict[str, Any]) -> ZeoConfig:
    """Merge a base ZeoConfig object with an override dictionary."""
    ...
    return ZeoConfig.model_validate(merged)
