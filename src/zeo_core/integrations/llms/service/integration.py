...
from zeo_core.integrations.llms import ChatMessage, LLMOptions
from zeo_core.integrations.llms.clients import LLMClient
...
class LLMIntegration(BaseIntegrationService):
    ...
    # src/zeocore/integrations/llms/service/integration.py
    ...
    def _extract_config(self) -> dict:
        ...
        Raises:
            ZeoIntegrationError: If configuration is invalid
        """
        if not getattr(self, "config", None):
            # Get default configuration
            if not self.config_provider:
                raise ZeoIntegrationError("Configuration provider not initialized")
        ...
        if self.config is None:
            raise ZeoIntegrationError("LLM configuration not initialized")

        # Validate configuration
        try:
            from zeo_core.integrations.llms.config import LLMConfig

            LLMConfig(**self.config)
            return self.config
        except Exception as e:
            # Make sure we explicitly raise ZeoIntegrationError for validation errors
            raise ZeoIntegrationError(f"Invalid LLM configuration: {e}") from e
    ...
    def get_client(self) -> LLMClient:
        ...
        Raises:
            ZeoIntegrationError: If the client is not initialized
        """
        if not getattr(self, "_initialized", False) or not self.client:
            raise ZeoIntegrationError("LLM client not initialized")
    ...
    def chat(
        ...
        """
        from zeo_core.integrations.llms.service.operations import chat
    ...
    def count_tokens(
        ...
        """
        from zeo_core.integrations.llms.service.operations import count_tokens
    ...
    def get_provider_status(self) -> list[dict] | None:
        ...
        Returns:
            list[dict] | None: Status information for all providers or None if
                not using fallback
        """
        from zeo_core.integrations.llms.service.operations import get_provider_status
