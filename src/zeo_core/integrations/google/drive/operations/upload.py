...
reading a file or obtaining file metadata are delegated to the ZeoCore FS API.
"""
...
from zeo_core.integrations.google.drive.utils.api import execute_api_request
...
def resolve_file_details(
    ...
    if not resolve_result.success or resolve_result.path is None:
        raise ZeoIntegrationError(f"Failed to resolve path: {resolve_result.error}")
    resolved_path = resolve_result.path

    # Get file information
    file_info = standalone.get_file_info(resolved_path)
    if not file_info.success or not file_info.exists:
        raise ZeoIntegrationError(f"File not found: {file_path}")
