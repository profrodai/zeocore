...
to the zeo_core.core.fs service functions.
"""
...
def _verify_html_structure(html_path: str, config: PandocConfig) -> None:
    ...
    Raises:
        ZeoIntegrationError: If the file cannot be read or has invalid
            HTML structure.
    """
    try:
        ...
        if not getattr(read_result, "success", False):
            ...
            raise ZeoIntegrationError(f"Could not read HTML file: {read_error}")
...
def _validate_input(html_path: str, config: PandocConfig) -> int:
    ...
    Raises:
        ZeoIntegrationError: If the input file is missing or has invalid structure.
    """
    file_info = fs.get_file_info(html_path)
    if not getattr(file_info, "success", False) or not getattr(
        file_info, "exists", False
    ):
        raise ZeoIntegrationError(f"Input file not found: {html_path}")
...
def _attempt_conversion(html_path: str, config: PandocConfig) -> str:
    ...
    Raises:
        ZeoIntegrationError: If the pandoc conversion fails.
    """
    try:
        pypandoc = importlib.import_module("pypandoc")
    except ImportError as e:
        raise ZeoIntegrationError(f"pypandoc module is not installed: {str(e)}") from e
    ...
    try:
        ...
        raise ZeoIntegrationError(f"Pandoc conversion failed: {str(e)}") from e
