...
to the zeo_core.core.fs service.
"""
...
from zeo_core.integrations.pandoc.models import ConversionMetrics, FileInfo
...
def verify_pandoc() -> str:
    ...
    Raises:
        ZeoIntegrationError: If pandoc is not installed.
    """
    try:
        import importlib
        ...
        logger.error(msg)
        raise ZeoIntegrationError(msg, {"module": "pypandoc"}) from err
    except OSError as err:
        msg = "Pandoc is not installed. Please install pandoc first."
        logger.error(msg)
        raise ZeoIntegrationError(msg, {"original_error": str(err)}) from err
