# ZeoCore Contracts (Ring A / Kernel)

## Mission

The `zeo_core.contracts` module defines the **canonical data contracts** for the ZeoCore system. This module represents **Ring A** in the Doctrine v3 architecture — the stable, versionable kernel that all other rings depend upon.
...
- No imports from `zeo_core.core.*`
- No imports from `zeo_core.tools.*`
- No imports from `zeo_core.integrations.*`
- No imports from `zeo_core.workflow.*`
...
- Other ZeoCore modules (except within contracts itself)
...
```python
# Correct usage
from zeo_core.contracts import CapabilityResult, ArtifactRef, RunManifest, StorageRef
