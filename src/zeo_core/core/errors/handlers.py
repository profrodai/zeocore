...
making it easier to diagnose and fix issues in the Zeo ecosystem.
"""
...
class ErrorHandler:
    ...
    def format_error(self, error: Exception) -> str:
        ...
        """
        if isinstance(error, ZeoError):
            return self._format_zeo_error(error)
        return str(error)

    def _format_zeo_error(self, error: ZeoError) -> str:
        """
        Format a ZeoError with context information.
        """
        message = str(error)
        result = [message]
