...
This module defines the base exception hierarchy for all errors in the ZeoCore
...
class ZeoError(Exception):
    """Base exception for all errors in the ZeoCore ecosystem."""
...
class ZeoFileNotFoundError(ZeoIOError):
...
class ZeoPermissionError(ZeoIOError):
    ...
    def __init__(
        ...
        # Bypass ZeoIOError init to set specific context manually
        ZeoError.__init__(
...
class ZeoFileExistsError(ZeoIOError):
...
class ZeoValidationError(ZeoError):
...
class ZeoFormatError(ZeoIOError):
...
class ZeoConfigurationError(ZeoError):
...
class ZeoBaseAuthError(ZeoError):
...
def _exception_converter(e: Exception) -> Exception:
    """
    Convert a standard exception to a ZeoCore custom exception.
    """
    if isinstance(e, ValueError):
        return ZeoValidationError(str(e), original_error=e)
    if isinstance(e, FileNotFoundError):
        path = getattr(e, "filename", None)
        return ZeoFileNotFoundError(path or "unknown", original_error=e)
    if isinstance(e, PermissionError):
        path = getattr(e, "filename", None)
        return ZeoPermissionError(path or "unknown", "access", original_error=e)
    if isinstance(e, FileExistsError):
        path = getattr(e, "filename", None)
        return ZeoFileExistsError(path or "unknown", original_error=e)
    if isinstance(e, IsADirectoryError):
        path = getattr(e, "filename", None)
        return ZeoIOError(f"Path is a directory: {path}", path, original_error=e)
    if isinstance(e, NotADirectoryError):
        path = getattr(e, "filename", None)
        return ZeoIOError(f"Path is not a directory: {path}", path, original_error=e)
    if isinstance(e, OSError):
        path = getattr(e, "filename", None)
        return ZeoIOError(str(e), path, original_error=e)
    return ZeoError(str(e), original_error=e)


def wrap_io_errors(func: Callable[..., R]) -> Callable[..., R]:
    """
    Decorator to wrap standard IO exceptions with ZeoCore's custom exceptions.
    """
