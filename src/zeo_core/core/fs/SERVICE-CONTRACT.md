...
- **`ZeoValidationError`** — the input cannot be coerced to a path (wrong type, malformed
  shape, or an otherwise-invalid value). The original error is chained as `original_error`.
- **`ZeoPathSecurityError`** and its subtypes **`ZeoPathEscapeError`** /
  **`ZeoPathOutsideBaseDirError`** — the path violates sandboxing (see §4). These are
...
normalization boundary raises `ZeoValidationError` on invalid input and
`ZeoPathSecurityError` on a sandbox violation.** A contract that claimed unconditional
...
- An **absolute** path is permitted **only if** it resolves to a location **inside**
  `base_dir`. An absolute path that escapes `base_dir` raises `ZeoPathOutsideBaseDirError`.
...
- Passing a malformed or sandbox-escaping path is a caller error and *will* raise
  (`ZeoValidationError` / `ZeoPathSecurityError`) — validate or sandbox-scope inputs,
