"""
Filesystem package for zeo_core.
...
from zeo_core.core.fs.results import (  # Typed results; Base results
    BoolResult,
    DataResult,
    DirectoryInfoResult,
    ErrorInfo,
...
from zeo_core.core.fs.service import FileSystemService, create_service, get_service
...
# `from zeo_core.core.fs._internal.x import y`, which this module transitively
...
#
# NOTE: this does NOT and cannot cover `from zeo_core.core.fs import _internal`
...
def __getattr__(name: str) -> Any:  # noqa: ANN401  # dynamic attr type
    ...
    if name.startswith("_internal") or name.startswith("_ops"):
        raise AttributeError(
            f"Module 'zeo_core.core.fs' has no attribute '{name}'. "
            f"Internal modules (_internal, _ops) are not part of the public API. "
            f"Use FileSystemService instead."
        )
    raise AttributeError(f"Module 'zeo_core.core.fs' has no attribute '{name}'")
