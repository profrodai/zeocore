...
from zeo_core.core.errors import ZeoValidationError
...
from zeo_core.core.fs.normalize import coerce_path
from zeo_core.core.fs.protocols import FsPathLike
from zeo_core.core.fs.results import ErrorInfo
...
class _BaseFileSystemService:
    ...
    def _normalize_input_path(self, path: FsPathLike) -> Path:
        ...
        except (ZeoPathEscapeError, ZeoPathOutsideBaseDirError) as e:
            # Re-raise sandbox violations to be mapped to specific error types
            raise e
        except ValueError as e:
            # Wrap standard coercion errors in ZeoValidationError
