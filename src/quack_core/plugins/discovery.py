...
Key principles:
- Discovery (listing) does NOT instantiate plugins
- Loading (explicit) instantiates and optionally registers plugins
...
class PluginInfo(QuackPluginMetadata):
    ...
    This ensures that all validated plugins have a stable identifier
...
class LoadResult(BaseModel):
    ...
    Success semantics:
    - Strict mode: success=True only if ALL requested plugins loaded cleanly
    - Non-strict mode: success=True if AT LEAST ONE plugin loaded
                       success=False only if ZERO plugins loaded (all failed)

    Warnings indicate non-fatal issues (missing plugins in non-strict mode,
...
class PluginLoader:
    ...
    Provides both discovery (listing available plugins without instantiation)
    and explicit loading (instantiating and optionally registering plugins).
    ...
    def _validate_plugin(
        ...
        """
        try:
            # Check for plugin_id (required for new plugins)
    ...
    def load_plugins(self, modules: list[str]) -> list[QuackPluginProtocol]:
        ...
        Note: This does NOT register the plugins. Use load_enabled_modules()
    ...
    def list_available_entry_points(
        ...
        This is pure discovery - no plugins are created or registered.
    ...
    def load_enabled_entry_points(
            self,
            enabled: list[str],
            group: str = "quack_core.plugins",
            strict: bool = True,
            auto_register: bool = True,
    ) -> LoadResult:
        """
        Load and optionally register only the specified plugins from entry points.

        This is the primary explicit loading API. It:
        1. Loads only the plugins specified in the enabled list
        2. Preserves the order of the enabled list
        3. Optionally registers plugins in the global registry
        ...
        Returns:
            LoadResult with success status, loaded plugins, warnings, and errors
        """
        from quack_core.plugins.registry import registry

        self.logger.info(
            f"Loading enabled plugins: {enabled} (strict={strict}, "
        ...
        # In strict mode, pre-validate all requested plugins exist
        if strict:
            missing = [pid for pid in enabled if pid not in ep_map]
            if missing:
                result.success = False
                result.errors.append(
                    f"Missing plugins in strict mode: {missing}. "
                    f"No plugins loaded (all-or-nothing)."
                )
                self.logger.error(result.errors[0])
                return result

        # Load plugins in the order specified
        for ep_name in enabled:
            ...
            try:
                ...
                if strict:
                    ...
                    # In strict mode, rollback all successfully registered plugins
                    if auto_register and registered_ids:
                        self.logger.warning(
                            f"Strict mode failure - rolling back {len(registered_ids)} "
                            f"registered plugins: {registered_ids}"
                        )
    ...
    def load_enabled_modules(
        ...
        """
        Load and optionally register plugins from explicit module paths.
        ...
        self.logger.info(
            f"Loading plugins from modules: {modules} (strict={strict}, "
            f"auto_register={auto_register})"
        )
        ...
        if result.success:
            ...
            self.logger.error(
                f"Failed to load plugins from modules. "
                f"Loaded: {len(result.loaded)}, Errors: {len(result.errors)}"
            )
    ...
    def discover_plugins(
        ...
        It instantiates plugins but does NOT register them.
...
def load_enabled_entry_points(
    ...
    """
    Load and optionally register only the specified plugins.

    This is the primary API for explicit plugin loading. It:
    - Loads only the plugins specified in the enabled list
    - Preserves the order of the enabled list
    - Optionally registers plugins in the global registry
