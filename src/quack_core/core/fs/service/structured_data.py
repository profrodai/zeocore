...
class StructuredDataMixin:
    ...
    def read_yaml(self, path: FsPathLike) -> DataResult[dict]:
        try:
            ...
            if not isinstance(data, dict):
                return DataResult(ok=False, success=False, path=normalized_path, data={}, format="yaml", error=f"YAML content was not a dictionary (got {type(data)})", message="Invalid YAML structure")
            return DataResult(ok=True, success=True, path=normalized_path, data=data, format="yaml", message="Read YAML")
    ...
    def write_yaml(self, path: FsPathLike, data: dict, atomic: bool = True) -> WriteResult:
        try:
            ...
            return WriteResult(ok=True, success=True, path=result_path, message="Wrote YAML")
        except Exception as e:
            safe_p_str = safe_path_str(path)
            safe_p = Path(safe_p_str) if safe_p_str else None
            return WriteResult(
    ...
    def read_json(self, path: FsPathLike) -> DataResult[dict]:
        try:
            ...
            if not isinstance(data, dict):
                return DataResult(ok=False, success=False, path=normalized_path, data={}, format="json", error=f"JSON content was not a dictionary (got {type(data)})", message="Invalid JSON structure")
            return DataResult(ok=True, success=True, path=normalized_path, data=data, format="json", message="Read JSON")
        except Exception as e:
            safe_p_str = safe_path_str(path)
            safe_p = Path(safe_p_str) if safe_p_str else None
            return DataResult(
    ...
    def write_json(self, path: FsPathLike, data: dict, atomic: bool = True, indent: int = 2) -> WriteResult:
        try:
            ...
            return WriteResult(ok=True, success=True, path=result_path, message="Wrote JSON")
        except Exception as e:
            safe_p_str = safe_path_str(path)
            safe_p = Path(safe_p_str) if safe_p_str else None
            return WriteResult(
