...
class PathOperationsMixin:
    ...
    def join_path(self, *parts: FsPathLike) -> DataResult[str]:
        try:
            if not parts:
                return DataResult(success=True, path=Path("."), data=".", format="path", message="Empty join")

            # Since join is string-based usually, we extract strings safely
            ...
            return DataResult(success=False, path=None, data="", format="path", error=str(e),
    ...
    def split_path(self, path: FsPathLike) -> DataResult[list[str]]:
        try:
            ...
            return DataResult(success=True, path=norm_path, data=components, format="path_components",
                              message=f"Split {len(components)} components")
        except Exception as e:
            safe_p = coerce_path_result(path)
            return DataResult(success=False, path=safe_p.path if safe_p.success else None, data=[],
                              format="path_components", error=str(e), message="Failed to split path")

    def normalize_path(self, path: FsPathLike) -> PathResult:
        try:
            norm_path = self._normalize_input_path(path)
            res_path = self.operations._normalize_path(norm_path)
            return PathResult(success=True, path=res_path, is_absolute=res_path.is_absolute(), is_valid=True,
                              exists=res_path.exists(), message=f"Normalized: {res_path}")
        except Exception as e:
            safe_p = coerce_path_result(path)
            return PathResult(success=False, path=safe_p.path if safe_p.success else None, is_valid=False, error=str(e),
    ...
    def expand_user_vars(self, path: FsPathLike) -> DataResult[str]:
        try:
            ...
            return DataResult(success=True, path=norm_path, data=expanded, format="path",
                              message=f"Expanded: {expanded}")
        except Exception as e:
            safe_p = coerce_path_result(path)
            return DataResult(success=False, path=safe_p.path if safe_p.success else None, data="", format="path",
                              error=str(e), message="Failed to expand variables")
    ...
    def is_same_file(self, path1: FsPathLike, path2: FsPathLike) -> DataResult[bool]:
        try:
            ...
            return DataResult(success=False, path=None, data=False, format="boolean", error=str(e),
    ...
    def is_subdirectory(self, child: FsPathLike, parent: FsPathLike) -> DataResult[bool]:
        try:
            ...
            return DataResult(success=False, path=safe_p.path if safe_p.success else None, data=False, format="boolean",
                              error=str(e), message="Failed to check subdirectory")
    ...
    def create_temp_directory(self, prefix: str = "quackcore_", suffix: str = "") -> DataResult[str]:
        try:
            ...
            return DataResult(success=True, path=temp_dir, data=str(temp_dir), format="path",
    ...
    def get_extension(self, path: FsPathLike) -> DataResult[str]:
        try:
            ...
            safe_p = coerce_path_result(path)
            return DataResult(success=False, path=safe_p.path if safe_p.success else None, data="", format="extension",
                              error=str(e), message="Failed to extract extension")

    def resolve_path(self, path: FsPathLike) -> PathResult:
        try:
            norm_path = self._normalize_input_path(path)
            res = self.operations._resolve_path(norm_path)
            return PathResult(success=True, path=res, is_absolute=res.is_absolute(), is_valid=True, exists=res.exists(),
