    ...
    def create_directory(self, path: FsPathLike, exist_ok: bool = True) -> OperationResult:
        try:
            ...
            return OperationResult(success=False,
                                   path=res.path if res.success else None, error=str(e),
    ...
    def get_file_info(self, path: FsPathLike) -> FileInfoResult:
        try:
            ...
            if not file_info.exists:
                return FileInfoResult(success=True, path=normalized_path, exists=False,
            ...
            res = coerce_path_result(path)
            return FileInfoResult(success=False, path=res.path if res.success else None,
                                  exists=False, error=str(e),
                                  message="Failed to get info")

    def list_directory(self, path: FsPathLike, pattern: str | None = None,
        ...
        try:
            ...
            res = coerce_path_result(path)
            return DirectoryInfoResult(success=False,
                                       path=res.path if res.success else None,
                                       error=str(e), message="Failed to list directory")

    def find_files(self, path: FsPathLike, pattern: str, recursive: bool = True,
                   include_hidden: bool = False) -> FindResult:
        try:
            normalized_path = self._normalize_input_path(path)
            files, directories = self.operations._find_files(normalized_path, pattern,
            ...
            res = coerce_path_result(path)
            return FindResult(success=False, path=res.path if res.success else None,
