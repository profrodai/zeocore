...
from quack_core.core.fs._operations.base import FileSystemOperations
from quack_core.core.fs.results import DataResult, OperationResult, WriteResult
...
class UtilityOperationsMixin:
    ...
    def ensure_directory(self, path: FsPathLike, exist_ok: bool = True) -> OperationResult:
        try:
            ...
            return OperationResult(success=False, path=safe_p.path if safe_p.success else None, error=str(e), message="Failed to ensure directory")
    ...
    def get_unique_filename(self, directory: FsPathLike, filename: str) -> DataResult[str]:
        try:
            ...
            return DataResult(success=False, path=safe_p.path if safe_p.success else None, data="", format="filename", error=str(e), message="Failed to generate filename")
    ...
    def create_temp_file(self, suffix: str = ".txt", prefix: str = "quackcore_", directory: FsPathLike | None = None) -> DataResult[str]:
        try:
            ...
            return DataResult(success=False, path=safe_p.path if safe_p and safe_p.success else None, data="", format="path", error=str(e), message="Failed to create temp file")
    ...
    def find_files_by_content(self, directory: FsPathLike, text_pattern: str, recursive: bool = True) -> DataResult[list[str]]:
        try:
            ...
            safe_p = coerce_path_result(directory)
            return DataResult(success=False, path=safe_p.path if safe_p.success else None, data=[], format="path_list", error=str(e), message="Search failed")
    ...
    def get_disk_usage(self, path: FsPathLike) -> DataResult[dict[str, int]]:
        try:
            ...
            safe_p = coerce_path_result(path)
            return DataResult(success=False, path=safe_p.path if safe_p.success else None, data={}, format="disk_usage", error=str(e), message="Failed to get disk usage")
    ...
    def get_file_type(self, path: FsPathLike) -> DataResult[str]:
        try:
            ...
            safe_p = coerce_path_result(path)
            return DataResult(success=False, path=safe_p.path if safe_p.success else None, data="", format="file_type", error=str(e), message="Failed to get file type")
    ...
    def get_file_size_str(self, size_bytes: int) -> DataResult[str]:
        try:
            ...
            return DataResult(success=False, path=None, data="", format="size_string", error=str(e), message="Formatting failed")
    ...
    def get_mime_type(self, path: FsPathLike) -> DataResult[str | None]:
        try:
            ...
            safe_p = coerce_path_result(path)
            return DataResult(success=False, path=safe_p.path if safe_p.success else None, data=None, format="mime_type", error=str(e), message="Failed to get mime type")
    ...
    def get_file_timestamp(self, path: FsPathLike) -> DataResult[float]:
        try:
            ...
            safe_p = coerce_path_result(path)
            return DataResult(success=False, path=safe_p.path if safe_p.success else None, data=0.0, format="timestamp", error=str(e), message="Failed to get timestamp")
    ...
    def compute_checksum(self, path: FsPathLike, algorithm: str = "sha256") -> DataResult[str]:
        try:
            ...
            safe_p = coerce_path_result(path)
            return DataResult(success=False, path=safe_p.path if safe_p.success else None, data="", format="checksum", error=str(e), message="Checksum failed")
    ...
    def is_path_writeable(self, path: FsPathLike) -> DataResult[bool]:
        try:
            ...
            safe_p = coerce_path_result(path)
            return DataResult(success=False, path=safe_p.path if safe_p.success else None, data=False, format="boolean", error=str(e), message="Check failed")
    ...
    def is_file_locked(self, path: FsPathLike) -> DataResult[bool]:
        try:
            ...
            safe_p = coerce_path_result(path)
            return DataResult(success=False, path=safe_p.path if safe_p.success else None, data=False, format="boolean", error=str(e), message="Lock check failed")
    ...
    def atomic_write(self, path: FsPathLike, content: str | bytes) -> WriteResult:
        try:
            ...
            safe_p = coerce_path_result(path)
            return WriteResult(success=False, path=safe_p.path if safe_p.success else None, error=str(e), message="Atomic write failed")