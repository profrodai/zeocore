# === QV-LLM:BEGIN ===
# path: quack-core/tests/test_capabilities/test_base.py
# role: tests
...
class CustomExtensionTool(BaseQuackToolPlugin):
    ...
    def __init__(self) -> None:
        # Patch get_service to avoid filesystem issues
        with patch('quack_core.lib.fs.service.get_service') as mock_get_service, \
                patch('quack_core.capabilities.base.setup_tool_logging') as mock_setup_logging, \
                patch('quack_core.capabilities.base.get_logger') as mock_get_logger, \
                patch('os.getcwd') as mock_getcwd:
            # Configure mocks
            mock_fs = create_mock_fs()
            mock_get_service.return_value = mock_fs
...
class TestBaseQuackToolPlugin(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        """
        with patch('quack_core.capabilities.base.setup_tool_logging'), \
                patch('quack_core.capabilities.base.get_logger'):
