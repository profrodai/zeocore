...
class TestBaseConfigProviderDiscovery:
    ...
    def test_load_config_discover(self) -> None:
        ...
        with patch.dict(os.environ, {"QUACK_TEST_CONFIG_CONFIG": "/env/config.yaml"}):
            with patch.object(provider, "_find_config_file") as mock_find:
                ...
                with patch(
                        "quack-core.fs.service.standalone.get_file_info") as mock_info:
                    mock_info.return_value.success = True
                    mock_info.return_value.exists = True

                    with patch(
                            "quack-core.fs.service.standalone.read_yaml") as mock_read:
        ...
        with patch.object(provider, "_find_config_file") as mock_find:
            ...
            with patch("quack-core.fs.service.standalone.get_file_info") as mock_info:
                ...
                with patch("quack-core.fs.service.standalone.read_yaml") as mock_read:
                    mock_read.return_value.success = True
                    mock_read.return_value.data = {
    ...
    def test_find_config_file(self) -> None:
        ...
        # Test with environment variable
        with patch.dict(os.environ, {"QUACK_TEST_CONFIG_CONFIG": "/env/config.yaml"}):
            with patch("quack-core.fs.service.standalone.expand_user_vars") as mock_expand:
                mock_expand.return_value = Path("/env/config.yaml")

                with patch(
                        "quack-core.fs.service.standalone.get_file_info") as mock_file_info:
                    ...
                    with patch(
                            "quack-core.paths.service.get_project_root",
        ...
        # Test with default locations
        with patch("quack-core.paths.service.get_project_root",
                   create=True) as mock_get_root:
        ...
        with patch("quack-core.paths.service.get_project_root",
            ...
            with patch("quack-core.fs.service.standalone.join_path") as mock_join:
    ...
    def test_resolve_path(self) -> None:
        ...
        with patch(
                "quack-core.fs.service.standalone.resolve_path"
        ) as mock_resolve:
