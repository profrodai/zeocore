...
def mock_paths_service(monkeypatch):
    ...
    if 'quack-core.paths' not in pytest.importorskip("sys").modules:
        import types
        temp_module = types.ModuleType('quack-core.paths')
        pytest.importorskip("sys").modules['quack-core.paths'] = temp_module
        temp_module.service = mock
    else:
        monkeypatch.setattr('quack-core.paths.service', mock)
